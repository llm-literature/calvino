...
export const LabyrinthLayout = ({ city, prevCity, nextCity, description, imageUrl, theme }: LayoutProps) => {
  return (
    <div className={cn("min-h-screen font-mono relative overflow-hidden", theme.colors.bg, theme.colors.text)}>
      {/* Grid Background */}
      <div 
      ...
      <div className="container mx-auto px-4 py-12 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
          ...
          <div className="md:col-span-3 flex flex-col justify-between h-full min-h-[50vh]">
            <div>
                <div className={cn("w-12 h-12 border-2 flex items-center justify-center mb-8", theme.colors.border)}>
            ...
            <div className="hidden md:block">
                 {prevCity && (
                    <Link href={`/city/${prevCity.type}/${prevCity.name}`} className="block mb-4 opacity-50 hover:opacity-100">
                        &lt; {prevCity.name.toUpperCase()}
                    </Link>
                 )}
                 {nextCity && (
                    <Link href={`/city/${nextCity.type}/${nextCity.name}`} className="block opacity-50 hover:opacity-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                ...
                >
                    <ReactMarkdown
                        components={{
                        p: ({ node, ...props }) => (
                            <p className={cn("mb-6 text-sm md:text-base leading-loose text-justify", theme.colors.text)} {...props} />
                        )
                        }}
                    >
                        {description}
